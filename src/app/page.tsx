...
export default function Home() {
  ...
  const sectionBtn = (key: string, label: string, icon: React.ReactNode, count?: number) => (
    <button
    ...
    >
      <div className="flex items-center gap-3">
        {icon}
        <span className="font-medium text-sm text-zinc-900 dark:text-zinc-50">{label}</span>
        {count !== undefined && <span className="text-xs text-zinc-400">({count})</span>}
      </div>
      <svg className={`w-4 h-4 text-zinc-400 transition-transform ${openSections[key] ? "rotate-180" : ""}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <main className="flex-1 overflow-y-auto">
          {!activeSubject ? (
            <div className="flex flex-col items-center justify-center h-full text-center p-8">
              ...
              <p className="text-zinc-500 dark:text-zinc-400 max-w-sm">Click the <span className="text-red-600 font-medium">+</span> button to create a subject.</p>
            </div>
          ) : !activeVideoId ? (
              ...
              {playlistProgress && (
                <div className="rounded-lg border border-blue-200 bg-blue-50 dark:border-blue-900 dark:bg-blue-950 px-4 py-3">
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-sm font-medium text-blue-700 dark:text-blue-300">Importing: {playlistProgress.title}</span>
                    <span className="text-xs text-blue-600 dark:text-blue-400">{playlistProgress.current}/{playlistProgress.total}</span>
                  </div>
                  <div className="w-full bg-blue-200 dark:bg-blue-800 rounded-full h-2"><div className="bg-blue-600 h-2 rounded-full transition-all" style={{ width: `${(playlistProgress.current / playlistProgress.total) * 100}%` }} /></div>
                </div>
              )}

              {error && <div className="rounded-lg border border-red-200 bg-red-50 dark:border-red-900 dark:bg-red-950 px-4 py-3 text-red-700 dark:text-red-300 text-sm">{error}</div>}

              {showSchedule && (
                <div className="rounded-xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 p-5">
                  <h3 className="text-sm font-semibold text-zinc-900 dark:text-zinc-50 mb-4">Weekly Schedule for {activeSubject.name}</h3>
                  <div className="space-y-2">
                    {activeSubject.schedule.map((entry, i) => (
                      <div key={entry.day} className="flex items-center gap-4">
                        ...
                        <input type="time" value={entry.startTime} onChange={(e) => updateSubjectSchedule(activeSubject.id, i, "startTime", e.target.value)} className="text-sm border border-zinc-300 dark:border-zinc-600 rounded px-2 py-1.5 bg-white dark:bg-zinc-800" />
                        <span className="text-zinc-400">to</span>
                        <input type="time" value={entry.endTime} onChange={(e) => updateSubjectSchedule(activeSubject.id, i, "endTime", e.target.value)} className="text-sm border border-zinc-300 dark:border-zinc-600 rounded px-2 py-1.5 bg-white dark:bg-zinc-800" />
                        {entry.startTime && entry.endTime && <span className="text-xs text-green-600 dark:text-green-400 font-medium">{getTimeDiff(entry.startTime, entry.endTime)}</span>}
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  {activeSubject.videos.map((video) => (
                    <div key={video.videoId} onClick={() => handleVideoClick(activeSubject.id, video)}
                      className="group flex items-center gap-4 p-4 rounded-xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 cursor-pointer hover:border-red-300 dark:hover:border-red-700 hover:shadow-md transition-all">
                      <img src={video.thumbnailUrl} alt="" className="w-24 h-14 rounded-lg object-cover flex-shrink-0" />
                      <div className="flex-1 min-w-0">
                        <h3 className="text-sm font-medium text-zinc-900 dark:text-zinc-50 truncate">{video.title}</h3>
                        {generatingPlan === video.videoId ? (
                          <div className="flex items-center gap-2 mt-1">
                            <svg className="animate-spin h-3 w-3 text-red-500" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" fill="none" /><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" /></svg>
                            <span className="text-xs text-red-500">Generating study plan...</span>
                          </div>
                        ) : video.studyPlan ? (
                          <div className="flex gap-2 mt-1">
                            ...
                            <span className="text-[10px] text-zinc-400">{video.studyPlan.quiz?.length || 0} quiz questions</span>
                          </div>
                        ) : (
            ...
            <div className="p-6 space-y-3">
              <button onClick={() => { setActiveVideoId(null); setOpenSections({}); setActiveChapter(null); }} className="text-sm text-red-600 dark:text-red-400 hover:underline mb-2">
                ← Back to {activeSubject.name}
              </button>

              <h2 className="text-lg font-bold text-zinc-900 dark:text-zinc-50 mb-4">{activeVideo.title}</h2>
              ...
              <div className="space-y-2">
                ...
                {openSections.summary && (
                  <div className="rounded-xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 p-5">
                    <p className="text-sm text-zinc-600 dark:text-zinc-400 leading-relaxed">{activeVideo.studyPlan.summary}</p>
                    <div className="mt-4 flex flex-wrap gap-2">
                      {activeVideo.studyPlan.keyTopics.map((t, i) => <span key={i} className="text-xs bg-zinc-100 dark:bg-zinc-800 text-zinc-700 dark:text-zinc-300 px-3 py-1 rounded-full">{t}</span>)}
                    </div>
                  </div>
                )}

                {sectionBtn("chapters", "Chapters", <svg className="w-5 h-5 text-orange-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 6h16M4 10h16M4 14h16M4 18h16" /></svg>, activeVideo.studyPlan.chapters.length)}
                {openSections.chapters && (
                  <div className="space-y-2">
                    {activeVideo.studyPlan.chapters.map((ch, i) => (
                      <div key={i} onClick={() => setActiveChapter(activeChapter === i ? null : i)}
                        className={`p-4 rounded-xl border cursor-pointer transition-all ${activeChapter === i ? "border-red-400 dark:border-red-600 bg-red-50 dark:bg-red-950" : "border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 hover:border-zinc-300"}`}>
                        <div className="flex items-center gap-3">
                        ...
                        {activeChapter === i && (
                          <div className="mt-3 ml-10">
                            <div className="flex flex-wrap gap-1 mb-2">{ch.keyConcepts.map((c, j) => <span key={j} className="text-[10px] bg-amber-100 dark:bg-amber-900 text-amber-700 dark:text-amber-300 px-1.5 py-0.5 rounded">{c}</span>)}</div>
                ...
                {sectionBtn("lastYear", "Last Year Important Notes", <svg className="w-5 h-5 text-amber-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>, activeVideo.studyPlan.lastYearNotes?.length)}
                {openSections.lastYear && activeVideo.studyPlan.lastYearNotes && (
                ...
                {sectionBtn("predicted", "Predicted Topics2025-2026", <svg className="w-5 h-5 text-purple-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 7h8m0 0v8m0-8l-8 8-4-4-6 6" /></svg>, activeVideo.studyPlan.predictedTopics?.length)}
                {openSections.predicted && activeVideo.studyPlan.predictedTopics && (
                  <div className="space-y-2">
                    {activeVideo.studyPlan.predictedTopics.map((topic, i) => {
                      ...
                      return (
                        <div key={i} className="p-4 rounded-xl bg-white dark:bg-zinc-900 border border-purple-200 dark:border-purple-800">
                          <div className="flex items-center gap-3 mb-2"><span className="w-6 h-6 rounded-full bg-purple-100 dark:bg-purple-900 text-purple-700 dark:text-purple-300 text-xs font-bold flex items-center justify-center">{i + 1}</span><h4 className="font-medium text-sm text-zinc-900 dark:text-zinc-50 flex-1">{topic.topic}</h4><span className={`text-[10px] px-2 py-0.5 rounded-full ${probColor}`}>{topic.probability}</span></div>
                ...
                {openSections.quiz && activeVideo.studyPlan?.quiz && (
                  <div className="rounded-xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 p-5">
                    <div className="flex gap-2 mb-4">
                      {(["all", "easy", "medium", "hard"] as const).map((f) => (
                        <button key={f} onClick={() => { setQuizFilter(f); setQuizAnswers({}); setQuizSubmitted(false); }}
                          className={`text-xs px-3 py-1 rounded-full transition-colors ${quizFilter === f ? f === "easy" ? "bg-green-500 text-white" : f === "medium" ? "bg-yellow-500 text-white" : f === "hard" ? "bg-red-500 text-white" : "bg-zinc-900 dark:bg-zinc-100 text-white dark:text-black" : "bg-zinc-100 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-400"}`}>
                    ...
                    {quizSubmitted && (
                      <div className={`mb-4 p-4 rounded-lg ${getQuizScore(activeVideo.studyPlan?.quiz, quizAnswers, quizFilter) >= 70 ? "bg-green-50 dark:bg-green-950 border border-green-200 dark:border-green-800" : "bg-yellow-50 dark:bg-yellow-950 border border-yellow-200 dark:border-yellow-800"}`}>
                        <p className="font-semibold text-lg">Score: {getQuizScore(activeVideo.studyPlan?.quiz, quizAnswers, quizFilter)}%</p>
                        <p className="text-sm text-zinc-600 dark:text-zinc-400">{getQuizCorrect(activeVideo.studyPlan?.quiz, quizAnswers, quizFilter)} / {getFilteredQuiz(activeVideo.studyPlan?.quiz, quizFilter).length} correct</p>
                      </div>
                    )}
                          ...
                          <div key={globalIdx} className="p-4 rounded-lg border border-zinc-200 dark:border-zinc-700 bg-zinc-50 dark:bg-zinc-800">
                            ...
                            <div className="ml-9 space-y-2">
                              {q.options.map((opt, j) => {
                                ...
                                let optClass = "border-zinc-200 dark:border-zinc-600 hover:border-zinc-300";
                                if (quizSubmitted && j === q.correctAnswer) optClass = "border-green-500 bg-green-50 dark:bg-green-950";
                                else if (quizSubmitted && isSelected && j !== q.correctAnswer) optClass = "border-red-500 bg-red-50 dark:bg-red-950";
                                ...
                                return (
                                  <button key={j} onClick={() => !quizSubmitted && setQuizAnswers({ ...quizAnswers, [globalIdx]: j })} disabled={quizSubmitted}
                                    ...
                                    <span className="font-medium text-zinc-500 mr-2">{String.fromCharCode(65 + j)}.</span>{opt}
                                    {quizSubmitted && j === q.correctAnswer && <span className="ml-2 text-green-600">✓</span>}
                                    {quizSubmitted && isSelected && j !== q.correctAnswer && <span className="ml-2 text-red-600">✗</span>}
                                  </button>
                                );
                              })}
                              {quizSubmitted && <div className="mt-2 p-3 rounded bg-zinc-100 dark:bg-zinc-700"><p className="text-xs text-zinc-600 dark:text-zinc-400"><strong>Explanation:</strong> {q.explanation}</p></div>}
                            </div>
                          </div>
                        );
                      })}
                    ...
                    <div className="mt-4 flex gap-3">
                      {!quizSubmitted ? (
                        ...
                        <button onClick={() => { setQuizAnswers({}); setQuizSubmitted(false); }} className="px-6 py-2.5 bg-zinc-200 dark:bg-zinc-700 text-zinc-700 dark:text-zinc-300 rounded-lg font-medium hover:bg-zinc-300 dark:hover:bg-zinc-600 transition-colors">
                          Retry
                        </button>
                      )}
                    </div>
